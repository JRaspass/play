...
RUN git clone -b 2019.11 git://github.com/rakudo/rakudo \
 && cd rakudo                                           \
...
CMD ["rakudo", "service.p6"]

FROM perl AS deps

RUN git clone git://github.com/ugexe/zef \
 && cd zef                               \
 && rakudo -I. bin/zef --/test install .
